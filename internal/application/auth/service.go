...
func (s *Service) Refresh(ctx context.Context, rawToken string) (Tokens, error) {
	...
	if err != nil || !stored.Valid(s.clock.Now()) || stored.Digest != digest(secret) {
		return Tokens{}, errors.New("invalid refresh token")
	}
	...
	if err := s.repository.UpdateRefreshToken(ctx, stored, version); err != nil {
		return Tokens{}, err
	}
	user, err := s.repository.FindUser(ctx, stored.UserID)
	if err != nil || !user.Active {
		return Tokens{}, errors.New("refresh token owner unavailable")
	}
	return s.issue(ctx, user)
}
