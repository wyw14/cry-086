...
func (s *Service) List(ctx context.Context, actorID, siteID, cursor string, limit int) ([]report.RegulatoryReport, string, error) {
	query := reportPageQuery{actorID: actorID, siteID: siteID, cursor: cursor, limit: limit}
	if err := s.authorizeReportPage(ctx, query); err != nil {
		return nil, "", err
	}
	page, err := s.loadReportPage(ctx, query)
	if err != nil {
		return nil, "", err
	}
...
func (s *Service) authorizeReportPage(ctx context.Context, query reportPageQuery) error {
	user, err := s.identities.FindUser(ctx, query.actorID)
	if err != nil {
		return errors.New("report listing is not authorized")
	}
	if !user.Active || !user.OwnsSite(query.siteID) {
		return errors.New("report listing is not authorized")
	}
	if !user.HasRole(identity.RoleSafetyOfficer, identity.RoleRegulator, identity.RoleViewer) {
		return errors.New("report listing is not authorized")
	}
	if query.limit < 1 || query.limit > 200 {
		return errors.New("report page size is invalid")
	}
	return nil
}
...
func (s *Service) loadReportPage(ctx context.Context, query reportPageQuery) (reportPage, error) {
	...
	page := reportPage{items: make([]report.RegulatoryReport, 0, len(items)), nextCursor: next}
